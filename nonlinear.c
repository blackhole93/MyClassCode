/**
Edits:
  - Started including halomodel in nonlinear_init (line 83)
  - Included halomodel in nonlinear_free -> no error! (line 206)
  - Started writing function nonlinear_halomodel (line 295)
    - Aim: include all quantities from backgroud and derived quantities
**/
...
int nonlinear_init(
  ...
  /** (b) for Halo Model non-linear spectrum */

  else if (pnl->method == nl_halomodel) {
    /*if (pnl->nonlinear_verbose > 0){
    ...
    if (pba->has_ncdm == _TRUE_) 
      fprintf(stdout,"Halo model does not include any wdm extensions up to now\n");
  }    

  /** (c) for HALOFIT non-linear spectrum */
