/*
Changes made:
- included method halomodel from nonlinear.c in line 152
- included function output_pk_hm --> gives linear Pk
- manipulated code s.t. one obtains the dimensionless pk as output file in line 971
*/
...
int output_init(
  ...
  if (ppt->has_pk_matter == _TRUE_) {
    ...
    class_call(output_halomodel(pba,psp,pop),
               pop->error_message,
               pop->error_message);
    
    /*class_call(output_halomodel1(pba,psp,pop),
               pop->error_message,
               pop->error_message);*/
    }
...
int output_halomodel1(
  ...
  for (index_z = 0; index_z < pop->z_pk_num; index_z++) {
    ...
    sprintf(file_name,"%s%s%s",pop->root,redshift_suffix,"halomodel_pk.dat");

    class_call(output_open_hm_pk_file(pba,
                                   psp,
                                   pop,
                                   &out,
                                   file_name,
    ...
    for (index_k=0; index_k<psp->ln_k_size; index_k++) {
      ...
      class_call(output_one_line_of_halomodelPk1(out,
                                       exp(psp->ln_k[index_k])/pba->h,
                                       pk_tot[index_k]*pow(pba->h,3)),
                 pop->error_message,
                 pop->error_message);
...
/**
 * This routines writes the output in files for halomodel.
 *
 * @param pba Input: pointer to background structure (needed for calling spectra_pk_at_z())
 * @param psp Input : pointer to spectra structure
 * @param pop Input : pointer to output structure
...
int output_halomodel(
		    struct background * pba,
		    struct spectra *psp,
		    struct output * pop){
  
  /* Write output file for nonlinear Power Spectrum */
  FILE * halomodelfilePk;
  FileName file_namePk;
  int l,j,index_halomodelPk;
  double z, k, p1h, p2h;

  sprintf(file_namePk,"%s%s",pop->root,"halomodel_Pk.dat");
  
  class_open(halomodelfilePk,file_namePk,"w",pop->error_message);
  fprintf(halomodelfilePk,"# One and two halo terms for given k & z\n");
  fprintf(halomodelfilePk,"#     z            k           P1h             P2h\n");

  for (j=0; j < psp->hm_z_slices; j++){
    for (l=0; l < psp->hm_k_slices; l++) {   
      z = psp->halomodel_pk[l*psp->hm_z_slices*psp->number_output_hm + j*psp->number_output_hm+psp->index_z_out];
      k = psp->halomodel_pk[l*psp->hm_z_slices*psp->number_output_hm + j*psp->number_output_hm+psp->index_k_out];
      p1h = psp->halomodel_pk[l*psp->hm_z_slices*psp->number_output_hm + j*psp->number_output_hm+psp->index_p1h_out];
      p2h = psp->halomodel_pk[l*psp->hm_z_slices*psp->number_output_hm + j*psp->number_output_hm+psp->index_p2h_out];
    
    
    class_call(output_one_line_of_halomodelPk(halomodelfilePk,
                                        z/*psp->halomodel_pk[l*psp->hm_z_slices*psp->number_output_hm + j*psp->number_output_hm+0]*/,
                                        k/*psp->halomodel_pk[l*psp->hm_z_slices*psp->number_output_hm + j*psp->number_output_hm+1]*/,
					p1h/*psp->halomodel_pk[l*psp->hm_z_slices*psp->number_output_hm + j*psp->number_output_hm+2]*/,
					p2h/*psp->halomodel_pk[l*psp->hm_z_slices*psp->number_output_hm + j*psp->number_output_hm+3]*/
                                        ),
               pop->error_message,
               pop->error_message);
    }
  ...
  fclose(halomodelfilePk);
  
  if (psp->hm_highest_order > 2){
  /* Write output file for nonlinear Bispectrum */
  FILE * halomodelfileBk;
  FileName file_nameBk;
  int index_halomodelBk;
  double b1h, b2h, b3h;

  sprintf(file_nameBk,"%s%s",pop->root,"halomodel_Bk.dat");
  
  class_open(halomodelfileBk,file_nameBk,"w",pop->error_message);
  fprintf(halomodelfilePk,"# One, two and three halo terms for given k & z\n");
  fprintf(halomodelfilePk,"#     z            k           B1h            B2h            B3h\n");

  for (j=0; j < psp->hm_z_slices; j++){
    for (l=0; l < psp->hm_k_slices; l++) {   
      z = psp->halomodel_pk[l*psp->hm_z_slices*psp->number_output_hm + j*psp->number_output_hm+psp->index_z_out];
      k = psp->halomodel_pk[l*psp->hm_z_slices*psp->number_output_hm + j*psp->number_output_hm+psp->index_k_out];
      b1h = psp->halomodel_pk[l*psp->hm_z_slices*psp->number_output_hm + j*psp->number_output_hm+psp->index_b1h_out];
      b2h = psp->halomodel_pk[l*psp->hm_z_slices*psp->number_output_hm + j*psp->number_output_hm+psp->index_b2h_out];
      b3h = psp->halomodel_pk[l*psp->hm_z_slices*psp->number_output_hm + j*psp->number_output_hm+psp->index_b3h_out];
    
    
    class_call(output_one_line_of_halomodelBk(halomodelfileBk,
                                        z/*psp->halomodel_pk[l*psp->hm_z_slices*psp->number_output_hm + j*psp->number_output_hm+0]*/,
                                        k/*psp->halomodel_pk[l*psp->hm_z_slices*psp->number_output_hm + j*psp->number_output_hm+1]*/,
					b1h/*psp->halomodel_pk[l*psp->hm_z_slices*psp->number_output_hm + j*psp->number_output_hm+2]*/,
					b2h/*psp->halomodel_pk[l*psp->hm_z_slices*psp->number_output_hm + j*psp->number_output_hm+3]*/,
					b3h
                                        ),
               pop->error_message,
               pop->error_message);
  ...
  fclose(halomodelfileBk);
  
  /* Write output file for linear Bispectrum */
  FILE * halomodelfileBklin;
  FileName file_nameBklin;
  int index_halomodelBklin;
  double bklin;

  sprintf(file_nameBklin,"%s%s",pop->root,"halomodel_Bk_lin.dat");
  
  class_open(halomodelfileBklin,file_nameBklin,"w",pop->error_message);
  fprintf(halomodelfilePk,"# Linear Bispectrum for given k & z\n");
  fprintf(halomodelfilePk,"#     z            k           B_lin\n");

  for (j=0; j < psp->hm_z_slices; j++){
    for (l=0; l < psp->hm_k_slices; l++) {   
      z = psp->halomodel_pk[l*psp->hm_z_slices*psp->number_output_hm + j*psp->number_output_hm+psp->index_z_out];
      k = psp->halomodel_pk[l*psp->hm_z_slices*psp->number_output_hm + j*psp->number_output_hm+psp->index_k_out];
      bklin = psp->bk_fixlinarray[j*psp->hm_k_slices+l];
    
    
    class_call(output_one_line_of_halomodelBklin(halomodelfileBklin,
                                        z,
                                        k,
					bklin
                                        ),
  ...
  fclose(halomodelfileBklin);
  
  
  /* Write output file for reduced linear Bispectrum */
  FILE * halomodelfileQlin;
  FileName file_nameQlin;
  int index_Qlin;
  double Qlin;

  sprintf(file_nameQlin,"%s%s",pop->root,"Q_123_tree.dat");
  
  class_open(halomodelfileQlin,file_nameQlin,"w",pop->error_message);
  fprintf(halomodelfileQlin,"# Reduced Tree level Bispectrum for given k & z\n");
  fprintf(halomodelfileQlin,"#     z            k           Q_tree_123\n");

  for (j=0; j < psp->hm_z_slices; j++){
    for (l=0; l < psp->hm_k_slices; l++) {   
      z = psp->halomodel_pk[l*psp->hm_z_slices*psp->number_output_hm + j*psp->number_output_hm+psp->index_z_out];
      k = psp->halomodel_pk[l*psp->hm_z_slices*psp->number_output_hm + j*psp->number_output_hm+psp->index_k_out];
      Qlin = psp->Q_123fixlinarray[j*psp->hm_k_slices+l];
    
    
    class_call(output_one_line_of_halomodelQlin(halomodelfileQlin,
                                        z,
                                        k,
					Qlin
                                        ),
  ...
  fclose(halomodelfileQlin);
  
  /* Write output file for reduced Bispectrum */
  FILE * halomodelfileQ;
  FileName file_nameQ;
  int index_Q;
  double Q;

  sprintf(file_nameQ,"%s%s",pop->root,"Q_123.dat");
  
  class_open(halomodelfileQ,file_nameQ,"w",pop->error_message);
  fprintf(halomodelfileQ,"# Reduced Bispectrum for given k & z\n");
  fprintf(halomodelfileQ,"#     z            k           Q_123\n");

  for (j=0; j < psp->hm_z_slices; j++){
    for (l=0; l < psp->hm_k_slices; l++) {   
      z = psp->halomodel_pk[l*psp->hm_z_slices*psp->number_output_hm + j*psp->number_output_hm+psp->index_z_out];
      k = psp->halomodel_pk[l*psp->hm_z_slices*psp->number_output_hm + j*psp->number_output_hm+psp->index_k_out];
      Q = psp->Q_123fixarray[j*psp->hm_k_slices+l];
    
    
    class_call(output_one_line_of_halomodelQ(halomodelfileQ,
                                        z,
                                        k,
					Q
                                        ),
  ...
  fclose(halomodelfileQ);
  
  }
  
  if (psp->hm_highest_order > 3){
    /* Write output file for nonlinear Trispectrum */
  FILE * halomodelfileTk;
  FileName file_nameTk;
  int index_halomodelTk;
  double t1h, t2h, t3h, t4h;

  sprintf(file_nameTk,"%s%s",pop->root,"halomodel_Tk.dat");
  
  class_open(halomodelfileTk,file_nameTk,"w",pop->error_message);
  fprintf(halomodelfileTk,"# One, two, three and four halo terms for given k & z\n");
  fprintf(halomodelfilePk,"#     z            k           T1h             T2h            T3h           T4h\n");

  for (j=0; j < psp->hm_z_slices; j++){
    for (l=0; l < psp->hm_k_slices; l++) {   
      z = psp->halomodel_pk[l*psp->hm_z_slices*psp->number_output_hm + j*psp->number_output_hm+psp->index_z_out];
      k = psp->halomodel_pk[l*psp->hm_z_slices*psp->number_output_hm + j*psp->number_output_hm+psp->index_k_out];
      t1h = psp->halomodel_pk[l*psp->hm_z_slices*psp->number_output_hm + j*psp->number_output_hm+psp->index_t1h_out];
      t2h = psp->halomodel_pk[l*psp->hm_z_slices*psp->number_output_hm + j*psp->number_output_hm+psp->index_t2h_out];
      t3h = psp->halomodel_pk[l*psp->hm_z_slices*psp->number_output_hm + j*psp->number_output_hm+psp->index_t3h_out];
      t4h = psp->halomodel_pk[l*psp->hm_z_slices*psp->number_output_hm + j*psp->number_output_hm+psp->index_t4h_out];
    
    
    class_call(output_one_line_of_halomodelTk(halomodelfileTk,
                                        z/*psp->halomodel_pk[l*psp->hm_z_slices*psp->number_output_hm + j*psp->number_output_hm+0]*/,
                                        k/*psp->halomodel_pk[l*psp->hm_z_slices*psp->number_output_hm + j*psp->number_output_hm+1]*/,
					t1h/*psp->halomodel_pk[l*psp->hm_z_slices*psp->number_output_hm + j*psp->number_output_hm+2]*/,
					t2h/*psp->halomodel_pk[l*psp->hm_z_slices*psp->number_output_hm + j*psp->number_output_hm+3]*/,
					t3h,
					t4h
                                        ),
               pop->error_message,
...
int output_one_line_of_halomodelPk(
                          FILE * halomodelfilePk,
                          double one_z,
			  double one_k,
                          double one_p1h,
  ...
  fprintf(halomodelfilePk,"%e  %e  %e  %e\n",one_z,one_k,one_p1h,one_p2h);
  
  return _SUCCESS_;

}

int output_one_line_of_halomodelPk1(
                          FILE * halomodelfilePk,
			  double one_k,
                          double one_pk
                          ) {

  fprintf(halomodelfilePk," %e  %e\n",one_k,one_pk);
  
  return _SUCCESS_;

}

int output_one_line_of_halomodelBk(
                          FILE * halomodelfileBk,
                          double one_z,
			  double one_k,
                          double one_b1h,
  ...
  fprintf(halomodelfileBk,"%e  %e  %e  %e  %e \n",one_z,one_k,one_b1h,one_b2h,one_b3h);
  
  return _SUCCESS_;

}

int output_one_line_of_halomodelBklin(
                          FILE * halomodelfileBklin,
                          double one_z,
			  double one_k,
                          double one_bklin
  ...
  fprintf(halomodelfileBklin,"%e  %e  %e\n",one_z,one_k,one_bklin);
  
  return _SUCCESS_;

}

int output_one_line_of_halomodelQ(
                          FILE * halomodelfileQ,
                          double one_z,
			  double one_k,
                          double one_Q
  ...
  fprintf(halomodelfileQ,"%e  %e  %e\n",one_z,one_k,one_Q);
  
  return _SUCCESS_;

}

int output_one_line_of_halomodelQlin(
                          FILE * halomodelfileQlin,
                          double one_z,
			  double one_k,
                          double one_Qlin
  ...
  fprintf(halomodelfileQlin,"%e  %e  %e\n",one_z,one_k,one_Qlin);
...
int output_one_line_of_halomodelTk(
                          FILE * halomodelfileTk,
                          double one_z,
			  double one_k,
                          double one_t1h,
  ...
  fprintf(halomodelfileTk,"%e  %e  %e  %e  %e  %e \n",one_z,one_k,one_t1h,one_t2h,one_t3h,one_t4h);
...
int output_halomodel(
  ...
  int index_hm_k;
  int index_hm_z;
  int index_hm_len = 4;*/ /* Amount of output values (currently z,k,1h,2h) */
  /*double * psp->halomodel_output;*/
/*
  sprintf(file_name,"%s%s",pop->root,"halomodel.dat");
  
  class_open(out,file_name,"w",pop->error_message);
  fprintf(out,"# Table of k, P_1h(k) and P_2h(k) for z = 0\n");
  fprintf(out,"# Computed with mass function of Sheth-Thormen\n");
  fprintf(out,"# z    k[1/Mpc]      P_1h        P_2h         \n");

  class_alloc(psp->halomodel_output,psp->hm_k_slices*psp->hm_z_slices*psp->number_output_hm*sizeof(double),psp->error_message);*/
  /*class_call(halomodel(pba,psp,ppm,ppt,1,2,2,1e-2,ppt->k_max_for_pk,psp->halomodel_output),psp->error_message,psp->error_message);*/
  
  /*
  for (index_hm_z=0; index_hm_z < psp->hm_z_slices; index_hm_z++) {
    for (index_hm_k=0; index_hm_k < psp->hm_k_slices; index_hm_k++) { 
      for (l=0; l < psp->hm_k_slices; l++) {   
    
      class_call(output_one_line_of_halomodel(out,
                                        psp->halomodel_output[l*psp->hm_z_slices*psp->number_output_hm + j*psp->number_output_hm+0],
                                        psp->halomodel_output[l*psp->hm_z_slices*psp->number_output_hm + j*psp->number_output_hm+1],
					psp->halomodel_output[l*psp->hm_z_slices*psp->number_output_hm + j*psp->number_output_hm+2],
					psp->halomodel_output[l*psp->hm_z_slices*psp->number_output_hm + j*psp->number_output_hm+3]
                                        ),
                 pop->error_message,
                 pop->error_message);
  ...
  free(psp->halomodel_output);

  return _SUCCESS_;

}

int output_one_line_of_halomodel(
                          FILE * halomodelfile,
                          double one_z,
                          double one_k,
                          double one_P_1h,
  ...
  fprintf(halomodelfile,"%f  %f  %f  %f  %f  %f\n",1.,2.,3.,4.);
...
int output_open_hm_pk_file(
  ...
  if (pop->write_header == _TRUE_) {
    fprintf(*pkfile,"# Matter power spectrum in halo model approach P(k) %sat redshift z=%g\n",first_line,z);
    fprintf(*pkfile,"# for k=%g to %g h/Mpc,\n",
            exp(psp->ln_k[0])/pba->h,
            exp(psp->ln_k[psp->ln_k_size-1])/pba->h);
    fprintf(*pkfile,"# number of wavenumbers equal to %d\n",psp->ln_k_size);
